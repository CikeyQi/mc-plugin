...
export class Update extends plugin {
  ...
  async runUpdate (isForce) {
    ...
    this.oldCommitId = await this.getcommitId('mc-plugin')
    uping = true
    ...
    /** 获取插件提交的最新时间 */
    const time = await this.getTime('mc-plugin')
    ...
    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      ...
      this.isUp = true
      /** 获取mc-plugin的更新日志 */
      const log = await this.getLog('mc-plugin')
  ...
  async getLog (plugin = '') {
    ...
    log = await this.makeForwardMsg(`mc-plugin更新日志，共${line}条`, log, end)
